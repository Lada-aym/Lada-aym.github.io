/* ==========================================================================
   LADA Samara — руководство по эксплуатации (app.js)
...
const LS_FAV = "ladaSamaraFavorites";
const LS_THEME = "ladaSamaraTheme";
...
/* Экранирование с белым списком: в текстах SECTIONS разрешён только <b>…</b>.
   Использовать ТОЛЬКО для текстового контента, не для атрибутов. */
function escB(value) {
  return esc(value)
    .replaceAll("&lt;b&gt;", "<b>")
    .replaceAll("&lt;/b&gt;", "</b>");
...
function renderBlock(block) {
  if (block.t === "p") {
    return `<p>${escB(block.v).replace(/\n/g, "<br>")}</p>`;
  }
  if (block.t === "h") {
    return `<h4 class="section-subtitle">${escB(block.v)}</h4>`;
  }
  if (block.t === "list") {
    return `<ul>${block.items.map(item => `<li>${escB(item)}</li>`).join("")}</ul>`;
  }
  if (block.t === "ol") {
    return `<ol>${block.items.map(item => `<li>${escB(item)}</li>`).join("")}</ol>`;
  }
  if (block.t === "warning") {
    const icon = block.icon || "⚠️";
    const level = block.level || "warning";
    const label = level === "danger" ? "ОПАСНО!" : "ВНИМАНИЕ!";
    return `<div class="warning warning-${esc(level)} ${esc(level)}"><strong>${icon} ${label}</strong><br>${escB(block.v).replace(/\n/g, "<br>")}</div>`;
  }
  if (block.t === "note") {
    return `<div class="note"><strong>Примечание:</strong><br>${escB(block.v).replace(/\n/g, "<br>")}</div>`;
  }
  if (block.t === "indicator") {
    ...
    return `<div class="indicator indicator-${esc(color)}">${iconHtml}<div class="indicator-body">${escB(block.v).replace(/\n/g, "<br>")}</div></div>`;
  }
  if (block.t === "switch") {
    const num = block.num || "";
    const iconSrc = block.icon || "";
    const iconHtml = iconSrc
      ...
      : (num ? `<span class="indicator-num">${esc(num)}</span>` : "");
    return `<div class="switch">${iconHtml}<div class="switch-body">${escB(block.v).replace(/\n/g, "<br>")}</div></div>`;
  }
  if (block.t === "figure") {
    ...
    return `<figure class="${classes}"${minWidth}>` +
      `<img src="${esc(block.src)}" alt="${esc(block.alt || block.caption || "")}" loading="eager" decoding="sync" fetchpriority="high">` +
      `${block.caption ? `<figcaption>${escB(block.caption)}</figcaption>` : ""}` +
      `</figure>`;
  }
  if (block.t === "table") {
    const renderCell = (cell, tag = "td") => {
      if (cell && typeof cell === "object") {
        const attrs = [];
        if (cell.colspan) attrs.push(`colspan="${esc(cell.colspan)}"`);
        if (cell.rowspan) attrs.push(`rowspan="${esc(cell.rowspan)}"`);
        // принудительный <th> внутри tbody для заголовков-групп
        const cellTag = cell.th ? "th" : tag;
        if (cell.th) attrs.push('class="table-section-header"');
        const raw = cell.text != null ? cell.text : "";
        const text = raw ? `<span>${escB(raw).replace(/\n/g, "<br>")}</span>` : "";
        const imgClass = cell.img && cell.img.indexOf("/jki/") >= 0 ? "table-img table-img-jki" : "table-img";
        const img = cell.img ? `<img class="${imgClass}" src="${esc(cell.img)}" alt="${esc(cell.alt || cell.text || "")}" loading="lazy">` : "";
        return `<${cellTag}${attrs.length ? " " + attrs.join(" ") : ""}>${text}${img}</${cellTag}>`;
      }
      return `<${tag}>${escB(cell).replace(/\n/g, "<br>")}</${tag}>`;
    };
    // headers может быть одномерным массивом (одна строка) или двумерным (несколько строк)
    const headerRows = Array.isArray(block.headers) && block.headers.length && Array.isArray(block.headers[0])
      ? block.headers
      : [block.headers];
    const theadHtml = headerRows.map(row => `<tr>${row.map(h => renderCell(h, "th")).join("")}</tr>`).join("");
    return `
      <div class="table-scroll">
        <table>
          ...
          <tbody>
            ${block.rows.map(row => `<tr>${row.map(cell => renderCell(cell, "td")).join("")}</tr>`).join("")}
          </tbody>
        </table>
      </div>`;
  }
...
function isTextBlock(b) {
  return b && (b.t === "p" || b.t === "list" || b.t === "ol");
}
function isWrapFigure(b) {
  return b && b.t === "figure" && b.class === "wrap";
}
function isWrapOrBareFigure(b) {
  return b && b.t === "figure" && (!b.class || b.class === "wrap");
}
function renderBlocks(blocks) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // две wrap-картинки подряд с текстом до/после — компонуем в multi (стопка справа)
    // текст + две фигуры подряд: текст слева, обе фигуры друг под другом справа
    const isFigFigText_bare = block.t === "figure" && !block.class && next && next.t === "figure" && !next.class
        && next2 && isTextBlock(next2);
    const isFigFigText_wrap = isWrapFigure(block) && next && isWrapFigure(next) && next2 && isTextBlock(next2);
    if (isFigFigText_bare || isFigFigText_wrap) {
      html.push(`<div class="figure-text-block multi">${renderBlock(next2)}${renderBlock(block)}${renderBlock(next)}</div>`);
      i += 2;
      continue;
    }
    const isTextFigFig_bare = isTextBlock(block) && next && next.t === "figure" && !next.class
        && next2 && next2.t === "figure" && !next2.class;
    const isTextFigFig_wrap = isTextBlock(block) && block.class !== "no-figure" && isWrapFigure(next) && next2 && isWrapFigure(next2);
    if (isTextFigFig_bare || isTextFigFig_wrap) {
      html.push(`<div class="figure-text-block multi">${renderBlock(block)}${renderBlock(next)}${renderBlock(next2)}</div>`);
      i += 2;
      continue;
    }
    // одна фигура wrap/bare + следующий текст: оборачиваем, чтобы текст обтекал
    const isFigText_bare = block.t === "figure" && !block.class && next && isTextBlock(next);
    const isFigText_wrap = isWrapFigure(block) && next && isTextBlock(next);
    if (isFigText_bare || isFigText_wrap) {
      html.push(`<div class="figure-text-block">${renderBlock(next)}${renderBlock(block)}</div>`);
      i++;
      continue;
    }
    // текст + фигура wrap/bare
    const isTextFig_bare = block.t === "p" && block.class !== "no-figure" && next && next.t === "figure" && !next.class;
    const isTextFig_wrap = isTextBlock(block) && block.class !== "no-figure" && isWrapFigure(next);
    if (isTextFig_bare || isTextFig_wrap) {
...
function renderStatus(items) {
  ...
  statusEl.textContent = `Все разделы руководства LADA Samara — всего ${total}.`;
}
