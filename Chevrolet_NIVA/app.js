...
function buildMenu() {
  if (typeof SECTIONS === "undefined") {
    console.warn("⚠️ SECTIONS не загружен. Меню будет построено после загрузки данных.");
    return;
  }
...
/* ---------------------------------------------------------------------------
   Инициализация — ждём загрузки SECTIONS из data.js
   --------------------------------------------------------------------------- */
function initApp() {
  if (typeof SECTIONS === "undefined") {
    console.warn("⚠️ SECTIONS ещё не загружен. Ждём data.js...");
    setTimeout(initApp, 100);
    return;
  }
  
  console.log("✅ SECTIONS загружен:", SECTIONS.length, "разделов");
  
  // Строим меню после загрузки SECTIONS
  buildMenu();
