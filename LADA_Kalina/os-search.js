/* ==========================================================================
   Поиск по сайту / руководству LADA Kalina
...
(function () {
  ...
  const MIN_QUERY = 2;
  const HISTORY_KEY = "ladaKalinaSearchHistory";
  ...
  function snippetFor(section, tokens) {
    // служебные теги <b>…</b> из SECTIONS в сниппете не нужны — убираем до вывода
    const raw = section.blocks.map(blockText).join(" ").replace(/<\/?b>/g, "").replace(/\s+/g, " ").trim();
    if (!raw) return "";
    ...
    overlay.innerHTML = `
      <div class="os-search-modal" role="dialog" aria-modal="true" aria-label="Поиск по руководству">
        <div class="os-search-head">
          <span class="os-search-title">Поиск по руководству LADA Kalina</span>
  ...
  function openSearch() {
    ...
    if (!resultsEl.innerHTML) {
      resultsEl.innerHTML = `<div class="os-search-empty">Поиск по ${getSections().length} разделам руководства LADA Kalina. Клавиша «/» — быстрый вызов поиска.</div>`;
    }
