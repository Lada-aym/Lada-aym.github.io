...
function initLazyScripts() {
  ...
  setTimeout(() => {
    ...
    if (!window.adsInitialized) {
      ...
      // 2. Подключаем основной загрузчик авторасстановки с вашим ID страницы
...
async function loadPage(url) {
  ...
  try {
    ...
    if (incomingContent) {
      ...
      // Перезапускаем авторасстановку Яндекса на новой SPA-странице
      if (window.Ya && window.Ya.Autoplacement) {
        try {
          window.Ya.Autoplacement.render(); 
          console.log('[OSApp РСЯ] Рекламные блоки авторасстановки обновлены.');
        } catch (yandexErr) {
