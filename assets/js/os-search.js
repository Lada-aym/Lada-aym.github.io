...
window.OSSearch = (() => {
    ...
    try {
      ...
      for (const [id, itemVector] of aiVectors.entries()) {
        ...
        if (similarity > 0.65) {
          ...
          if (!isAbsoluteHome && currentSection && item.url) {
            const cleanItemUrl = cleanPathForContext(item.url);
            if (!cleanItemUrl.startsWith(currentSection)) continue;
          }
  ...
  function search(query) {
    ...
    let results = [...matchedIds].map(id => {
      ...
      if (isAbsoluteHome) return true;
      if (currentSection && item.url) {
        const cleanItemUrl = cleanPathForContext(item.url);
        return cleanItemUrl.startsWith(currentSection);
      }
      return true;
    });
  ...
  function navigateTo(targetUrl) {
    ...
    if (lowerUrl === "/" || lowerUrl === "" || lowerUrl.endsWith("index.html") || lowerUrl.includes("garage") || lowerUrl === "index") {
